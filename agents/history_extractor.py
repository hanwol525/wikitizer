...
Capturing date_text:
Each event may optionally include a "date_text" field. Capture a date_text when the text states WHEN the event happened as a point in time. Two kinds count:
- an explicit in-world date or year -- "342 AR", "in the Third Age", "the year 1247", "Second Age, year 88"; and
- a present-relative offset, i.e. how long BEFORE NOW it happened -- "200 years ago", "about 40 years ago", "two centuries ago".
Copy the date string EXACTLY as it appears, whichever kind it is: do NOT convert it to a number, do NOT normalize the format, do NOT compute or subtract anything, and do NOT guess a date that isn't stated. If the event has no stated date of either kind, omit "date_text" or set it to null. Most events have no stated date, and that is expected and fine.

Important -- these are NOT a date_text (leave them inside the description and set "date_text" to null):
- a DURATION, i.e. how LONG something lasted rather than when it happened: "a 30-year war", "30 years of fighting", "a feud that ran for two centuries". A span is not a point in time.
- a clue relative to ANOTHER EVENT rather than to now: "after the old kingdoms collapsed", "before the Empire fell", "during the reign of X".
