...
- Do NOT spawn a separate people for every sub-group of a people already present. "X-born dwarves", "the dwarves of Y", "stronghold dwarves" are the SAME people seen in a narrower light — fold them into the parent people (or record them as a detail on it), unless the sub-group is itself a distinctly named, standalone culture. And a single unnamed individual (one background farmer) is not a people or culture at all — that is at most a character, and a lone unnamed passer-by is usually nothing.

For each people or culture, extract:
- name: the group's primary/canonical name (or your short descriptive label if it has no proper name). Use the fullest proper form the messages give and put shorter forms in aliases; a synthesized label must be plainly descriptive and drawn only from what the text states, never a jokey or made-up proper name.
...
- Return ONE entry per distinct people or culture in this response — gather its facts from every message in this batch into a single entry; do not split one described group into several entries or list the same one twice. Matching peoples up across different batches is handled later.
