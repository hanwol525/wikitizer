...
Skip one-off, joke, or hypothetical mentions — a passing exclamation or a "what if X" aside is not an organization. And never build an organization by stitching together place, realm, or character names that merely share one sentence: "The Taken Lands of the Dwarven Stronghold, known as Tiber" is not one organization — those are Locations (and, where a person is named, a character), captured by their own extractors. Only capture a group the text actually presents AS a group.
...
- name: the organization's primary/canonical name (or your short descriptive label if it has no proper name). Use the fullest proper form the messages give — keep a full title ("the Order of the Dawn", not "the Order") — and put shorter forms in aliases; a synthesized label must be plainly descriptive and drawn only from what the text states, never a jokey or made-up proper name.
...
- Return ONE entry per distinct organization in this response — gather its facts from every message in this batch into a single entry; do not split one described group into several entries or list the same one twice. Matching organizations up across different batches is handled later.
