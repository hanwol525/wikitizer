"""Pydantic v2 models for the shared eval result envelope (mirrors ``fc-result.schema.json``).

These models ARE the result contract on the Python side: an emitter builds a result,
pydantic validates it on construction, and ``model_dump(mode="json", by_alias=True)``
produces the schema-valid JSON. Keeping the schema and these models in lock-step means a
mismatch fails loudly at emit time rather than silently shipping an off-spec result.

The pieces here are eval-agnostic (``Status`` / ``Engine`` / ``Evidence`` / the generic
``Item`` / the graders / ``Summary``); a per-eval result object (e.g. FC's ``FCResult``)
assembles them and lives in that eval's package.
...
Python:
  ...
  * ``Summary.n_pass`` -- the JSON key is ``"pass"``, but ``pass`` is a reserved Python
    ...
    ``by_alias=True`` so the wire key is still ``"pass"``.
...
class Item(BaseModel):
    """One criterion's grading result. ``id`` is the ONLY join key (the dotted criterion
