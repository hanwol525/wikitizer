...
def test_verbatim_check_drops_non_matching_quote_keeps_siblings(caplog):
    ...
    assert len(result) == 1
    assert [d.text for d in result[0].details] == ["Forged in dragonfire"]
...
def test_verbatim_check_tolerates_multiline_message():
    ...
    assert len(result) == 1
    assert [d.text for d in result[0].details] == ["Seals the rift"]
...
def test_identical_quotes_deduped_but_details_kept():
    ...
    assert len(result) == 1
    assert [d.text for d in result[0].details] == ["Ancient", "Very old"]   # details NOT deduped
...
def test_detail_with_non_string_detail_or_quote_dropped(caplog):
    ...
    assert len(result) == 1
    assert [d.text for d in result[0].details] == ["A sword"]
    assert any(
